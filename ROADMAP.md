...
### Épic 5 — API REST
Endpoints FastAPI (`app/api/reports.py`) : `POST /reports/{weekly,monthly}` (déclenchement manuel, hors planification), `GET /reports/tasks/{task_id}` (statut Celery), `GET /reports/history` (liste `report_runs`, filtrable par type), `GET /reports/{run_id}/download` (URL présignée MinIO). Documentation OpenAPI générée automatiquement (`/docs`). Le lien de téléchargement est signé avec un endpoint S3 public distinct de l'endpoint interne (`s3_public_endpoint_url` ≠ `s3_endpoint_url`) — sans ça l'URL présignée contient `minio:9000`, résoluble seulement dans le réseau Docker, inutilisable par un client externe.
