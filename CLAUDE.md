...
Aucun code applicatif n'existe encore. Le dépôt contient uniquement la roadmap (`ROADMAP.md`) et les jeux de données source (`/data`). **`ROADMAP.md` est la source de vérité** pour l'architecture cible, la stack et le découpage en épics — le lire avant toute implémentation.
...
- La couverture n'est jamais 1:1 entre les trois fichiers (363 zones ATMO vs 1228 communes géo vs 1219 lignes EPCI) — prévoir des communes sans indice ATMO plutôt que de supposer une couverture exhaustive.
...
- Le référentiel communal combinant ces trois sources doit être matérialisé une fois au démarrage plutôt que rejoint à chaque génération de rapport (`ROADMAP.md` §3.4, Épic 2).

## Version control

Ce dépôt utilise GitButler (`but`), pas Git directement, pour les commits/branches/PR.
